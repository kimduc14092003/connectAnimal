...
public class AlgorithmEndless : MonoBehaviour
{
    ...
    private void Start()
    {
        listCell=gameObject.GetComponent<EndlessModeController>().listCell;
    }
    ...
    private bool CheckLineX(int y, int x1, int x2)
    {
        ...
        {
            if (listCell[y][x].GetComponent<CellController>().CellID != "0")
            {
    ...
    private Vector2Int[] CheckRectX(Vector2Int p1, Vector2Int p2)
    {
        ...
        //======== Code cua tui ==================
        for (int x = pMinX.x; x < listCell[0].Count; x++)
        {
            // check three line
...
/*                print("Rect X: (" + pMinX.x + "," + pMinX.y + ") -> ("
    ...
    private Vector2Int[] CheckRectY(Vector2Int p1, Vector2Int p2)
    {
        ...
        for (int y = pMinY.y; y < listCell.Count; y++)
        {
